...
def make_one_event(i, ch, day_idx, eventDate, weekday):
    user_idx = np.random.randint(1,5)
    userId = f"user_{user_idx}"
    ...
    if user_idx==1:
        base+=0.08
        if 6<=notif_hour<10: base+=0.07
        if 20<=notif_hour<=22: base-=0.03
    elif user_idx==2:
        base+=0.02
        if 18<=notif_hour<=22: base+=0.06
        if 6<=notif_hour<8: base-=0.05
    elif user_idx==3:
        base+=0.0
        if 13<=notif_hour<18: base+=0.05
    elif user_idx==4:
        base-=0.05
        if 20<=notif_hour<=22: base+=0.05
        if 6<=notif_hour<10: base-=0.05
    energyKwh=0
...
def simulate_events(n_days=180):
    rows=[]
    idx=0
...
def main():
    ...
    if len(speed_events)>0:
        ...
        X_sp=pd.get_dummies(X_sp_raw,columns=["category","challengeId","userId"],drop_first=True)
    ...
    def recommend_speed():
        if speed_model is None or X_sp is None: return None
        ...
        df=pd.DataFrame(rows)
        df=pd.get_dummies(df,columns=["category","challengeId","userId"],drop_first=True)
